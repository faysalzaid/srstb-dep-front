...
function ProjectLists(props) {
    ...
    useEffect(()=>{
            const date = new Date()
            let data = []
            const response = axios.get(`${url}/pr-list`).then((resp)=>{
              if(resp.data.error){
                
              }else{
                data = resp.data.prlist
                const newData = data.filter((d)=>{return d.year.slice(0,4)==date.getFullYear()})
                
                setProjectLists(newData)
                setProjectCount(resp.data.prCount)
              }
    ...
    useEffect(()=>{
      setFetchedResult(searchTerm.length<1?projectLists:searchResult)
    },[projectLists,searchTerm])
  ...
  const addProject =async(e)=>{
    ...
    if(projectForm.name==="" || projectForm.status===""||projectForm.description===""||projectForm.year===""){
      setErrorMessage('Please Provide all data')
       ...
       const response = await axios.post(`${url}/pr-list`,projectForm).then((resp)=>{
        console.log('New Data',resp.data);
        if(resp.data.error){
          setErrorMessage(resp.data.error)
        }else{
            setProjectLists([...projectLists,resp.data])
            setProjectForm({name:"",status:"",description:"",year:""})
...
const deleteClient =async(ids)=>{
  const response = await axios.get(`http://localhost:4000/clients/delete/${ids}`).then((resp)=>{
    ...
    if(resp.data.error){
      ...
      const newData = projectLists.filter((c)=>c.id!==ids)
      setProjectLists(newData)
    ...
    return ( 
      ...
      <div className='mb-5'>
        ...
        {/* End of Info cards */}
        <div>
          <Button onClick={openModal}>Register New Project</Button>
        </div>
        <Modal isOpen={isModalOpen} onClose={closeModal}>
          ...
          <Label>
            <span>Name</span>
              <Input type="text" className="mt-1" name="name" placeholder="Full Name"  autoComplete='off' onChange={(e)=>setProjectForm({...projectForm,name:e.target.value})}/>
          </Label>
          <Label>
            <span>Year</span>
              <Input type="date" className="mt-1" name="year" placeholder="Email"  autoComplete='off' onChange={(e)=>setProjectForm({...projectForm,year:e.target.value})}/>
          </Label>
          <Label>
          <span>description</span>
            <Textarea type="text" className="mt-1" name="description" placeholder="Description"  autoComplete='off' onChange={e=>setProjectForm({...projectForm, description:e.target.value})}/>
        </Label>
        

          <Label className="mt-4">
          <span>Status</span>
          <Select className="mt-1" name="status"  onChange={(e)=>setProjectForm({...projectForm,status:e.target.value})}>
        ...
        <TableContainer className="mb-8">
          <Table>
            ...
            <TableBody>
              {fetchedResult.map((prj, i) => (
                <TableRow key={i}>
                  <TableCell>
                    <div className="flex items-center text-sm">
                      
                      <div>
                        <p className="font-semibold">{prj.name}</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400">{prj.job}</p>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <span className="text-sm">{prj.year.slice(0,4)}</span>
                  </TableCell>
                  
                  
                  <TableCell>
                  <Badge type={prj.status==='active'?"success":"danger"}>{prj.status}</Badge>
                </TableCell>
                  
                  <TableCell>
                    <div className="flex items-center space-x-4">
                      <Link to={{pathname:`/app/pr-list/${prj.id}`}}>
                      <Button layout="link" size="icon" aria-label="Edit">
                        <EditIcon className="w-5 h-5" aria-hidden="true" />
                      </Button>
                      </Link>
                      <Button onClick={()=>deleteClient(prj.id)} style={{color:'red'}} layout="link" size="icon" aria-label="Delete">
