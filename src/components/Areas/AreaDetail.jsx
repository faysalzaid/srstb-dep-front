...
function AreaDetail(props) {
    ...
    useEffect(()=>{
      const getData = async()=>{
        await axios.get(`${url}/area/${id}`,{withCredentials:true}).then((resp)=>{
    ...
    //   console.log('areaForm',areaForm);
        ...
        await axios.post(`${url}/area/${id}`,request,{withCredentials:true}).then((resp)=>{
            if(resp.data.error) return setOpenError({open:true,message:`${resp.data.error}`})
            setAreaData(resp.data)
            closeModal()
    ...
    const deleteArea = async()=>{
      await axios.delete(`${url}/area/delete/${id}`,{withCredentials:true}).then((resp)=>{
        if(resp.data.error){
    ...
    return ( 
      ...
      />
        ...
        <Modal isOpen={isModalOpen} onClose={closeModal}>
          ...
          <Label>
            <span>Name</span>
              <Input type="text" value={areaForm.name} className="mt-1" name="name" placeholder="Designation Name"  autoComplete='off' onChange={(e)=>setAreaForm({...areaForm,name:e.target.value})}/>
          </Label>
                ...
                <TableRow>
                  <TableCell>
                    <div className="flex items-center text-sm">
                      ...
                      <div>
                        <p className="font-semibold">{AreaData.name}</p>
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-4">
                      ...
                      <Button onClick={()=>openDelete(AreaData.id)}  style={{color:'red'}} layout="link" size="icon" aria-label="Delete">
                        <TrashIcon className="w-5 h-5 ml-4" aria-hidden="true" />
                      </Button>
                    </div>
                  </TableCell>
                </TableRow>
